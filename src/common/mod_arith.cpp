...
void vector_mul_mod_hybrid(const u64 modulus, const size_t vec_len, const u64 *f,
    ...
    for (int i = 0; i < vec_len; i++) {
        ...
        u64 hi_temp = (a + u) >> mshift;
        u64 hi_temp_temp = (u128)hi_temp * mont_harvay >> 64;
        h[i] = (u128)hi_temp * mont - (u128)hi_temp_temp * modulus;
    }
}
...
void vector_mul_mod_barrett(const u64 modulus, const size_t vec_len,
    ...
    for (int i = 0; i < vec_len; i++) {
        ...
        h[i] = a - approx_mod_multiple;
        h[i] -= (h[i] >= modulus) ? modulus : 0;
    }
}
